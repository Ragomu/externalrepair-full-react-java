...
interface EditFormProps {
  ...
  loading?: boolean;
  showRelatedNf?: boolean;
  disabledFields?: { quantity?: boolean; date?: boolean };
}
...
const EditForm = ({
  ...
  const isQuantityValid = disabledFields?.quantity
    ? true
    : Boolean(returnQuantity && returnQuantity > 0);
  const isDateValid = Boolean(returnDate && returnDate.trim() !== '');
  const isNfValid = Boolean(returnNf && returnNf.trim() !== '');
  const isFormValid = Boolean(isDateValid && isNfValid && isQuantityValid);

  return (
    <RowFieldsContainer>
      <InputsContainer ref={inputsContainerRef}>
        <ColumnField>
          <FieldLabel>Quant. de retorno</FieldLabel>
          <EditableFieldWrapper>
            <EditableFieldInput
              type="number"
              value={returnQuantity}
              onChange={(e) => setReturnQuantity(Number(e.target.value))}
              required={!disabledFields?.quantity}
