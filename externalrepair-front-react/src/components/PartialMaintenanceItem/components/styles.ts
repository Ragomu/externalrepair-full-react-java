...
export const RowFieldDiscardReport = styled.div`
  display: flex;
  flex-direction: row;
  align-items: center;
  gap: 8px;
...
export const InputDisposalReportContainer = styled.div<{ hasFile?: boolean }>`
  display: flex;
  align-items: center;
  justify-content: center;
  gap: 8px;
  ...
  width: 170px;
  height: 25px;
  border: 1px solid #3153dd;
  border-radius: 6px;
  padding: 6px 16px;
  background: ${({ hasFile }) => (hasFile ? '#3153dd' : '#fff')};
  color: ${({ hasFile }) => (hasFile ? '#fff' : '#3153dd')};
  font-weight: 400;
  font-size: 14px;
  cursor: pointer;
  transition: all 0.2s ease;
